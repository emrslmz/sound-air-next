          ...
          <div class="w-full p-8 my-4 md:px-12 lg:w-9/12 lg:pl-20 lg:pr-40 mr-auto rounded-2xl shadow-2xl bg-zinc-200 dark:bg-gray-800">
            <div class="py-10">
              <h1 class="font-extrabold leading-10 tracking-tight text-left text-gray-900 dark:text-white sm:leading-none text-4xl sm:text-5xl md:text-6xl lg:text-5xl">
                <span class="inline md:block dark:text-dark-200">{{ $t("contactUs") }} <span>👇</span> </span>
                <span class="relative py-3 text-transparent bg-clip-text bg-gradient-to-br from-indigo-600 to-indigo-500 md:inline-block">{{ $t("forYourQuestions") }} </span>
              </h1>
            </div>
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2 mt-5">
              <input class="w-full bg-gray-100 dark:bg-gray-400 text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline" type="text" :placeholder="$t('name')" v-model="model.name" />
              <input class="w-full bg-gray-100 dark:bg-gray-400 text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline" type="text" :placeholder="$t('lastName')" v-model="model.surname" />
            </div>
            <input class="w-full bg-gray-100 dark:bg-gray-400 text-gray-900 mt-4 p-3 rounded-lg focus:outline-none focus:shadow-outline" type="email" :placeholder="$t('email')" v-model="model.email" />

            <div class="my-2">
              <textarea :placeholder="$t('yourMessage')" class="w-full h-32 bg-gray-100 dark:bg-gray-400 text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline" v-model="model.message"></textarea>
            </div>
            <div class="my-2 w-1/2 lg:w-1/4">
              <button class="inline-flex items-center justify-center w-full px-8 py-4 text-base font-bold leading-6 text-white bg-indigo-600 border border-transparent rounded-full md:w-auto hover:bg-indigo-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-600" @click="postContact(model)">{{ $t("sent") }}</button>
            </div>
          </div>

          <div class="w-full lg:-mt-96 lg:w-2/6 px-8 py-12 mb-20 ml-auto bg-indigo-600 rounded-2xl">
            <div class="flex flex-col text-white">
              <h1 class="font-bold uppercase text-4xl my-4">{{ $t("or") }}</h1>
              <p class="text-gray-200">{{ $t("youCanReachUsBySendingEmail") }}</p>
              ...
              <div class="flex my-4 w-2/3 lg:w-1/2">
                ...
                <div class="flex flex-col">
                  <h2 class="text-2xl">{{ $t("socialMedia") }}</h2>
